...
float4 frag(v2f i, uint facing: SV_IsFrontFace): COLOR
{
    ...
    if (float(1))
    {
        ...
        float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, TRANSFORM_TEX(poiMesh.uv[float(4)], _MainTex) + _Time.x * float4(0,0,0,0));
        half3 detailMask = 1;
        ...
        clip(OutlineMask * float(4.03) - 0.001);
        ...
        if(float(1) == 1)
        {
            col.rgb = lerp(col.rgb, mainTexture.rgb, float(0.142));
        }
